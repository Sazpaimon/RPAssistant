...
public class UsbDriverService extends Service implements UsbDriverListener {
    ...
    public UsbDriverService() {
        super();
    }

    @Override
    public void reportControllerState(int controllerId, SparseIntArray buttonEvents, float leftStickX, float leftStickY, float rightStickX, float rightStickY, float leftTrigger, float rightTrigger) {
        // Call through to the client's listener
        if (listener != null) {
            listener.reportControllerState(controllerId, buttonEvents, leftStickX, leftStickY, rightStickX, rightStickY, leftTrigger, rightTrigger);
        }
    }
    ...
    public static boolean shouldClaimDevice(UsbDevice device) {
        return ((!kernelSupportsXboxOne() || !isRecognizedInputDevice(device)) && XboxOneController.canClaimDevice(device)) ||
